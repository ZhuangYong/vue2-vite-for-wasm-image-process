<template>
  <div class="dialogs">
    <!--修改canvas尺寸-->
    <el-dialog v-model="editCanvasSizeDialog" width="300px" title="画布大小">
    ...
    <!--新建-->
    <el-dialog v-model="newDialog" width="300px" title="新建图片">
      <el-form label-width="46px">
        <p class="title">新建大小</p>
        <div class="block">
          <el-form-item label="宽：">
      ...
      <template #footer>
        <el-button type="primary" @click.stop.prevent="newPicture">确定</el-button>
      </template>
    </el-dialog>

    <!--输入文字-->
    <el-dialog v-model="textDialog" title="文字" width="600px">
      <el-form>
        <el-form-item label="">
      ...
      <template #footer>
        <el-button size="small" type="primary" @click.stop.prevent="onEditTextSure">确定</el-button>
      </template>
    </el-dialog>
  ...
  methods: {
    ...
    onEditTextSure() {
      if ((this.editText || '').replace(/ /g, '')) {
        ...
        imageHelper.addText(this.editText, {left, top, fontFamily: FontHelper.fontOptions[0].value })
        this.editText = ''
