export function triggerDragAndDrop (source, target, _dragStartEvent, offsetY = 0) {
  ...
  const fireDragEvent = function (type, elem, clientX, clientY, dataTransfer) {
    ...
    Object.defineProperty(event, 'dataTransfer', {
      get() {
        return dataTransfer || createNewDataTransfer()
      }
    })
  ...
  const elemDrag = typeof source === 'string' ? document.querySelector(source) : source
  const elemDrop = typeof target === 'string' ? document.querySelector(target) : target
